...
class FisbaReadyBeam:
    ...
    def __init__(self, port="/dev/ttyUSB0", baud=57600, timeout=3, address=0, debug=0):
        """
        Parameters
        ----------
        port           : str
    ...
    def send_command(self, command):
        ...
        if response_frame[6] == "+":  # Detect errors and raise Exception
            ...
            error = "Error signaled by device: {0}, {1}".format(
                error_nr, self.device_errors[error_nr]
            )
    ...
    def set_brightness(self, power=[10.0, 0.0, 10.0]):
        """
        ...
        for i in range(len(power)):
            ...
            command = self.construct_command(
                7013, value=power[i] * 1.0, instance=int(i + 1)
            )
            self.send_command(command)
        if self.debug >= 1:
