...
class ComplexUNet(nn.Module):
    ...
    def __init__(
        self, size, kernel_size=3, bn_flag=False, CB_layers=[3, 3, 3], FM_num=[4, 8, 16]
    ):
        """
        ...
        self.linear1 = ComplexLinear(in_features=self.H, out_features=self.W)
        self.linear2 = ComplexLinear(in_features=self.H, out_features=self.W)
    ...
    def forward(self, x):
        ...
        # x0_down4x = self.linear2( x0_down4x.view(-1, 8, 150*150) ).view(-1, 8, 150, 150)
        ...
        #
        # x4 = self.linear2(x4.permute(0, 1, 3, 2)).permute(0, 1, 3, 2)
...
class ComplexResNet(nn.Module):
    def __init__(
        self, size, kernel_size=3, bn_flag=False, CB_layers=[4, 4, 4], FM_num=[4, 4, 4]
    ):
    ...
    def forward(self, x):
        ...
        x_out = torch.stack(
            [torch.real(x_out).double(), torch.imag(x_out).double()], dim=0
        )
        return x_out
